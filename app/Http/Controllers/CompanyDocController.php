      ...
      if(file_exists(storage_path('documents/templates/' . $type . '.docx'))) {
        ...
        switch ($type) {
            ...
            case 'smff-coverage-certification':
                ...
                $template->setValue('companyName',htmlspecialchars( $company->name));
                $template->setValue('issueDate', $issueDate);
                $vessels = $company->vessels()->select('id', 'name')->where('active_field_id', 2)->orderBy('name')->whereNull('deleted_at')->get();
                $columns = 4;
            ...
            case 'schedule-a-non-tank':
                ...
                if($company->qi_id > 0){
                    $qi = Company::where('id',$company->qi_id)->whereNull('deleted_at')->first();
                    $template->setValue('qiName', htmlspecialchars($qi->name));
                } else {
                    $template->setValue('qiName', ' // ');
                }
                $vessels = $company->vessels()->where('active_field_id', 2)->where('tanker', 0)->with('type:id,name')->orderBy('name')->whereNull('deleted_at')->get();
                ...
                break;
            case 'appendix-II-djsa' : 
                $fileName = str_slug($company->name) . ' - Appendix II DJSA - ' . date('m-d-Y_h_ia');
                $template->setValue('companyName', htmlspecialchars($company->name));
                $template->setValue('issueDate', $issueDate);
                $vessels = Vessel::where([['active_field_id', 3], ['company_id', $company->id]])->whereNotNull('plan_id')->orderBy('name')->whereNull('deleted_at')->get();
                $template->cloneRow('cVesselName', count($vessels));
            ...
            case 'schedule-a-tanker':
                ...
                $vessels = $company->vessels()->where('active_field_id', 2)->where('tanker', 1)->orderBy('name')->whereNull('deleted_at')->get();
            ...
            case 'schedule-a-combined':
                ...
                $vessels = $company->vessels()->where('active_field_id', 2)->with('type:id,name')->orderBy('name')->whereNull('deleted_at')->get();
                $template->cloneRow('cVesselName', count($vessels));
                for ($row = 1, $rowMax = count($vessels); $row <= $rowMax; $row++) {
                    $name = $vessels[$row - 1]->name;
                    $template->setValue('cVesselName#' . $row, htmlspecialchars($name));
