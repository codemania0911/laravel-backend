...
class Company extends BaseModel
{
    ...
    const FIELDS_COMPANY = '
        c1.id, 
        c1.active_field_id AS djs_active, 
        c1.has_photo, 
        c1.networks_active, 
        c1.vendor_active, 
        c1.shortname, 
        cs.status AS capabilies_active, 
        c1.name AS name, 
        cs.id IS NOT NULL AS resource_provider, 
        c1.active_field_id, 
        cs.id AS smff_service_id,
        p0.qi_id, 
    ...
    const UNION_FIELDS_COMPANY ='
    c1.id, p1.id AS vrpid, 0 AS vrp_import, c1.active_field_id AS djs_active, c1.networks_active, c1.vendor_active, cs.id IS NOT NULL AS capabilies_active, p1.primary_smff AS vrp_primary_smff, "Vendor" AS vendor_category, p1.id IS NOT NULL AS linked, 1 AS djs, p1.status = "Authorized" AS auth, c1.name AS name, p1.plan_holder AS vrp_plan_name, p0.plan_number, p0.plan_number AS vrp_plan_number, cs.id IS NOT NULL AS resource_provider, c1.active_field_id, p1.status AS vrp_status, c1.active_field_id AS coverage, cs.id AS smff_service_id, p1.plan_type AS vrp_plan_type, p1.holder_country AS vrp_country, p0.qi_id, operating_company_id, email, fax, phone, website, description, c1.created_at, c1.updated_at';
    ...
    const UNION_FIELDS_PLAN = '
        ...
        p2.primary_smff AS vrp_primary_smff, 
        "" AS vendor_category, 
        0 AS linked, 0 AS djs, 
        p2.status = "Authorized" AS auth, 
        plan_holder AS name, 
        plan_holder AS vrp_plan_name, 
        p2.plan_number, 
        p2.plan_number AS vrp_plan_number, 
        0 AS resource_provider, 
        0 AS active_field_id, 
        p2.status AS vrp_status, 
        plan_holder LIKE "%donjon%" AS coverage, 
        0 AS smff_service_id, 
        plan_type AS vrp_plan_type, 
        p2.holder_country AS vrp_country, 
        0 AS qi_id, 
        0 AS operating_company_id, 
        "" AS email, 
        "" AS fax, 
        "" AS phone, 
        "" AS website, 
    ...
    const FIELDS_ADDRESS = '
        company_addresses.id AS id, 
        company_addresses.zone_id, 
    ...
    {
        return $this->hasMany(CompanyAccounting::class);
    }

    public function plans()
    {
        return $this->hasMany(Plan::class)->whereNull('deleted_at');
    }
    ...
    public function vesselBillingGroup()
    {
        return $this->hasMany(VesselBillingGroup::class);
    }
}
